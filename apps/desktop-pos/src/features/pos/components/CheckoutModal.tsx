...
export function CheckoutModal({ isOpen, onClose, onComplete }: CheckoutModalProps) {
  // Local State
  // const [tenderedAmount, setTenderedAmount] = useState<string>(''); // Removed in favor of hook
  ...
  // Calculations
  // Parse hook's display value (handle empty string safely)
  const tenderedAmountCents = parseFloat(displayValue || '0') * 100;
  ...
  }, [isOpen, grandTotal, startEditing]);
  ...
  const handleComplete = async () => {
    ...
    // Ensure value is cleanly formatted/validated before processing (optional safety)
    ...
    try {
      ...
      // Send to Electron
      // @ts-expect-error - Types might be slightly mismatch on id which is generated by backend
  ...
  }, [isOpen, canComplete, isProcessing, onClose, handleComplete]);
